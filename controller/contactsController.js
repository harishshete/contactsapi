...
exports.updateContact = async (req,res)=>{
  ...
  // console.log(`[${Date.now() - startTime}ms] Starting to update in the database`);
        ...
        if (req.params.pass_string == 'perforce'){
            ...
            console.log(id);
            try {
                const updatedContact = await Contact.updateOne({_id:id},{$set: { name:name, email:email, phone:phone } },{ new: true });
...
exports.updateContact = async (req,res)=>{
        ...
        if (req.params.pass_string == 'perforce'){
            ...
            //console.log(id);
            try {
                const updatedContact = await Contact.findOneAndUpdate({_id:req.body.id},{ $set: { name: req.body.name, email: req.body.email, phone: req.body.phone } },{ new: true });
                console.log("Printing the value of updatedContact\n"+updatedContact);
                res.status(200).json(updatedContact);
            } catch (error) {
                res.status(500).json("Could Not Update " + error)
